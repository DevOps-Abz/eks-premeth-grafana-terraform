...
# I will build an EKS cluster (control plane), then set up OIDC trust for pods, then installs add-ons, 
# and provisions worker nodes (on-demand + spot) in my VPC, all done via variables, for code reusable purposes.

# This creates the EKS Control Plane cluster and sets its name, permissions (IAM role), and Kubernetes version using variables.
...
# tls cert is fetched from the eks cluser the was initially created, so AWS can trust it for pod IAM roles
data "tls_certificate" "eks_certificate" {
  url = aws_eks_cluster.eks[0].identity[0].oidc[0].issuer
}

# Creates IAM Role via OIDC and links EKS cluster to it so pods get their granular permission (without full IAM Role Node perm).
